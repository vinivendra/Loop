...
class DialView: NSView {
	...
	func resizeViews() {
		...
		middle.frame = NSRect(x: border, y: border,
		                      width: width - 2 * border, height: height - 2 * border)
		inside.frame = NSRect(x: border + thickness,
		                      y: border + thickness,
		                      width: width - 2 * (border + thickness),
		                      height: height - 2 * (border + thickness))
		center.frame = NSRect(x: 2 * border + thickness,
		                      y: 2 * border + thickness,
		                      width: width - 2 * (2 * border + thickness),
		                      height: height - 2 * (2 * border + thickness))
	}
}
